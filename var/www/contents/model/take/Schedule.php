<?php
	...
	*/
	...
	var $group = '*';
		...
		*	@params $date 現在日 $id 曜日のID
		*	@return 配列データ
		*/
		function getWeekData($date, $id, $cID=0){
		...
		*/
		...
		/*
			*	勤務数の取得
			*/
		function getWork($tid, $arrayWork, $type='hour'){
			$this->addQuery('take_base_id', $tid);
			$this->addQuery('type', 1);

			$this->addQuery('(0');			
			foreach ($arrayWork as $item){
				$this->addQuery('OR week', $item);
			}
			$this->addQuery('OR 0)');			
		...
		function getWorkArray($tid, $dateStart, $dateEnd, $dayType=1, $type='day'){
			$count = 0;
			
			for ($i = strtotime($dateStart); $i <= strtotime($dateEnd); $i+=(3600 * 24)){
				$w = date('w', $i);
				
				if (empty($arrayWeekCount[$w])){$arrayWeekCount[$w] = 0;}
				$arrayWeekCount[$w]++;
			}
			
			if ($dayType == 1){$arrayWork = array(1,2,3,4,5);}
			if ($dayType == 2){$arrayWork = array(0,6);}

			//各曜日を何回やったか掛け算する
			foreach ($arrayWork as $week){
				$count+= $this->getWork($tid, array($week), $type)->getCount() * $arrayWeekCount[$week];
			}
...
/*		
			...
			//日付から曜日が何個か合ったかの計算 
					...
					$arraySalary['arrayList'][$item['id']][$key2] = '';
	
					if (($key2 == 'dataA') || ($key2 == 'dataB') || ($key2 == 'dataC')){

				

						
						$arraySalary['arrayList'][$item['id']]['arrayTake'] = $item;							
						
						if (getVar($arraySalary, $key2 . 'Max') < $arraySalary['arrayList'][$item['id']][$key2]){
							$arraySalary[$key2 . 'Max'] = $arraySalary['arrayList'][$item['id']][$key2];
						}
					}
				}
			}
