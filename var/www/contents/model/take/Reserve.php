<?php
	...
	*/
	...
	//var $group = 'take_reserve.timeStart';
		...
		*/
		function getDataTime($cID, $date, $time, $skypeTime, $type, $tID=0, $cshID=0){			
			...
			$arrayWeek = $MemberBase->getWeek($week, $cshID);
			if (isset($week2) ){
				$arrayWeek = array_merge($arrayWeek, $MemberBase->getWeek($week2, $cshID));							
			}
			...
			if ($type == 1){
				if ($this->isMemberCheck){
					if (empty($this->isB)){
						...
						$MemberBase->addQuery('OR timeStart', $time);
						
						if ($skypeTime == 50){
							$MemberBase->addQuery('OR timeStart', date('H:30:00', strtotime($time)));							
						}
			...
			$countWeek = 99;			
			if ($cshID){
				$arraySch = $this->getFunctionData('Schedule');
				$arraySchWeek = explode(',', $arraySch[$cshID]['week']);
				$countWeek = count($arraySchWeek);
				$TakeSchedule = $this->getModel('take/Schedule');
				$TakeSchedule->target = 'COUNT(*) as count,take_base_id';
				$TakeSchedule->group = 'take_base_id';
				$TakeSchedule->addQuery('time', $time);
				$TakeSchedule->addQuery('type', 1);
				
				$TakeSchedule->addQuery('(0');
				foreach ($arraySchWeek as $week){
					$TakeSchedule->addQuery('OR week', $week);
				}
				
				$TakeSchedule->addQuery('1)');
				
				$dbGetWeek = $TakeSchedule->getData();

				while ($objWeek = $dbGetWeek->getData()){
					$arrayTakeWeek[$objWeek['take_base_id']] = $objWeek['count'];
				}	
			}
			foreach ($arrayTake as $item){
				if (!$arrayJudg = getVar($arrayReserve, $item['id']) ){				
					...
					if (empty($arrayMember[$item['id']])){
						...
						if ((!isset($arrayTakeWeek)) || (getVar($arrayTakeWeek, $item['id']) >= $countWeek)){
