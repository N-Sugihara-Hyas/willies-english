...
*/
		...
		if (!$arrayData = $this->MemberSetting->getSettingStep(6, $this->arrayAll) ){
			...
			if ($arrayMember['cource_base_id'] == 1){
				$levelChild = intval(getVar($arrayData, 'levelChild'));

				if (strlen($levelChild)){
					$this->arrayUser['level'] = $arrayMember['level'] = intval(getVar($arrayData, 'levelChild'));
					$this->arrayUser['levelChild'] = $arrayMember['levelChild'] = intval(getVar($arrayData, 'levelChild'));
			...
			if (!$this->arrayUser['isSetting']){
				...
				//if (($this->arrayUser['state'] < 3) || ($this->arrayUser['state'] == 11) || ($this->arrayUser['state'] == 10)){
				if (($this->arrayUser['state'] == 11) || ($this->arrayUser['state'] == 10)){
					$this->MemberSetting->setSession('trialout', 'mypage/setting/end/');
...
//			$this->TakeReserve->addQuery('concat(date," ",timeStart) >=', date('Y-m-d H:i:s'));
			$this->TakeReserve->addQuery('date >=', $arrayData['dateFirst']);
